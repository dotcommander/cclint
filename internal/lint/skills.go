...
func validateBasicSkillFields(fmData map[string]any, filePath, contents string) []cue.ValidationError {
	...
	if version, ok := fmData["version"].(string); ok && version != "" {
		if err := ValidateSemver(version, filePath, textutil.FindFrontmatterFieldLine(contents, "version")); err != nil {
...
func validateSkillDescription(description, filePath, contents string) []cue.ValidationError {
	...
	if len(description) > 250 {
		out = append(out, cue.ValidationError{
			File:     filePath,
			Message:  fmt.Sprintf("Description is %d chars, exceeding the 250-character limit. Skill descriptions over 250 chars are truncated by Claude Code.", len(description)),
			Severity: "warning",
			Source:   cue.SourceAnthropicDocs,
			Line:     textutil.FindFrontmatterFieldLine(contents, "description"),
		})
