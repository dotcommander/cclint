...
- v2.1.232–v2.1.237 swept (2026-08-19): modeled settings `spellcheck` (v2.1.235, open struct) and marketplace aliases `additionalMarketplaces`/`allowedMarketplaces` (v2.1.232). Bugfix-only: 2.1.233/234/236/237. Binary-verified: map-form marketplaces (extraKnown/additional) wrap the source union in a `source` field (record value {source, installLocation?, autoUpdate?}); array forms (strict/blocked/allowed) take the flat union. Not modeled: wrapper installLocation/autoUpdate as typed sub-fields (open struct covers) and the alias both-set ignore+rewrite behavior (runtime, no static key).
...
|-----|-------|
| claude_code_last_updated | v2.1.237 (binary installed: v2.1.237) |
