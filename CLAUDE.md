...
**Marketplace sources**: github, git, git-subdir, url, npm, file, directory, hostPattern, settings, archive
...
- v2.1.182–v2.1.183 swept (2026-06-18). Bugfix-only gap, NO lint surface. v2.1.182 is a skipped/absent changelog header (binary shipped ahead, never backfilled — like prior no-entry headers 2.1.155/2.1.164/2.1.171/2.1.177/2.1.180). v2.1.183 is entirely runtime/UI/subagent bugfixes (startup line removal, thinking.disabled 400 fix, WebSearch in subagents, vim cursor, TUI corruption, empty-thinking-block re-prompt, autocomplete dedupe, MCP auth-stub tools, tmux teammate panes, background-task lifecycle, scheduled-task notification classification, focus-mode hook timing). Classified in-drive against the full lintable surface: NO new settings.json keys, KnownTools, hook events/handler fields, model aliases, enum values, or plugin/agent/command/skill frontmatter fields. Binary installed: v2.1.183. Re-sweep 2.1.182 if its changelog header ever backfills.
- v2.1.184–v2.1.185 swept (2026-06-20). Bugfix/UI-only gap, NO lint surface. v2.1.184 is a skipped/absent changelog header (continues the no-entry-header pattern: 2.1.155/2.1.164/2.1.171/2.1.177/2.1.180/2.1.182). v2.1.185 is a single UI/runtime bullet — the stream-stall hint was reworded ("Waiting for API response · will retry in …" replacing "No response from API · Retrying in …") and its trigger delay raised from 10s to 20s; pure wording + timing, no static config/schema/tool surface. Classified in-drive against the full lintable surface: NO new settings.json keys, KnownTools, hook events/handler fields, model aliases, enum values, or plugin/agent/command/skill frontmatter fields. Binary installed: v2.1.185. Re-sweep 2.1.184 if its changelog header ever backfills.
- v2.1.186–v2.1.204 swept (2026-07-08). 15 changelog headers present (2.1.188/189/192/194 absent, continuing the no-entry-header pattern of 2.1.155/164/171/177/180/182/184). Four lintable items, all new settings.json keys verified against the v2.1.204 binary's zod schemas and modeled in settings.cue: respondToBashCommands (v2.1.186, bool, top-level — keeps context-only behavior after an !-bash command), sandbox.credentials (v2.1.187, nested object under sandbox — files:[...{path,mode:"deny"}], envVars:[...{name,mode:"deny"|"mask",injectHosts?}], allowPlaintextInject:bool — blocks sandboxed commands from credential files / secret env vars), autoMode.classifyAllShell (v2.1.193, bool sub-key of autoMode — routes all Bash/PowerShell commands through the auto-mode classifier), workflowSizeGuideline (v2.1.202, top-level enum small|medium|large|unrestricted — controls dynamic-workflow agent-count target). All four were passing unchecked before (settings.cue open struct; sandbox/autoMode carry trailing `...`); now type-validated. Verified: TestValidateSettings new cases (4 valid + 1 negative enum rejection of "huge") + go build/test ./internal/cue/ ./internal/lint/ green (commit 6ac8c6a). Dropped per guardrail: teammateMode "iterm2" value (teammateMode already modeled as permissive-string enum), skill-frontmatter kebab/snake/camelCase acceptance for display-name/default-enabled/fallback/metadata (runtime case-folding over already-modeled fields, no new schema keys), the "Dynamic workflow size" /config picker UI (underlying key workflowSizeGuideline WAS modeled — only the UI is out of scope), and the v2.1.196–v2.1.203 background-session/daemon reliability flood (runtime/UI, no static config surface). No new KnownTools, hook events/handler fields, model aliases, global enum values (colors/permission modes/memory scopes/marketplace sources), or plugin/agent/command/skill frontmatter fields. Bugfix-only versions in the gap: 2.1.190, 2.1.197, 2.1.201.
- v2.1.205–v2.1.217 swept (2026-07-22). 13 changelog headers (2.1.213 absent — continues the no-entry-header pattern of 2.1.155/164/171/177/180/182/184/188/189/192/194). Six lintable items verified against the v2.1.217 binary's zod schemas (via bounded `strings | grep -oE`) and modeled: five new settings.json keys in settings.cue — disableAutoMode (v2.1.207, top-level enum — only accepted value is the literal "disable", turns auto mode off; distinct from the autoMode.* classifier sub-struct), axScreenReader (v2.1.208, bool — opt-in plain-text rendering for screen-reader users), vimInsertModeRemaps (v2.1.208, open map `{...}` — two-key insert-mode sequences like jj→Escape in vim mode; binary shape v.record(string,unknown)), emojiCompletionEnabled (v2.1.217, bool — emoji shortcode autocomplete in the prompt input), sandbox.filesystem.disabled (v2.1.216, bool sub-key of a new sandbox.filesystem object — skip filesystem isolation while keeping network/seccomp; the filesystem object also carries pre-existing unmodeled siblings allowWrite/denyWrite/denyRead/allowRead/allowManagedReadPathsOnly, left open via `...`); plus one new KnownTool EndConversation (v2.1.214 — feature-gated EndConversationTool; added to Go KnownTools, auto-propagates into the generated CUE #KnownTool union, no CUE/test edit). All six were passing unchecked before (settings.cue open struct; sandbox carries trailing `...`); the five settings keys are now type-validated. Verified: TestValidateSettings_v2217Fields (5 valid + 2 negative, incl. enum rejection of disableAutoMode:"enable") + TestKnownToolUnionMatchesSource + go build/test ./internal/cue/ ./internal/textutil/ green (commits 2e6a051, b0870cd). Dropped per guardrail: the v2.1.205–v2.1.216 UI/runtime/CLI-flag/env-var-only flood (spinner, TUI, startup, --flags, CLAUDE_CODE_* env vars incl. CLAUDE_CODE_MAX_WEB_SEARCHES_PER_SESSION / _MAX_SUBAGENTS_PER_SESSION / _MCP_AUTO_BACKGROUND_MS — none are settings.json keys) and the skill auto-invocation behavior change (v2.1.215, runtime); the emoji-shortcode / screen-reader / vim-remap runtime UI wiring is out of scope (the underlying settings keys WERE modeled). No new hook events, hook handler fields, model aliases, global enum values (colors/permission modes/memory scopes/marketplace sources), or plugin/agent/command/skill frontmatter fields in this window. Bugfix/UI/env-only versions in the gap: 2.1.205, 2.1.206, 2.1.209, 2.1.210, 2.1.211, 2.1.212, 2.1.215.

- v2.1.218–v2.1.226 swept (2026-08-08). The installed binary is v2.1.224. Modeled the v2.1.218 skill `background` field and case-insensitive Claude boolean literals (`yes`/`no`/`on`/`off`/`1`/`0`), v2.1.219 `claude-opus-5`, `sandbox.network.strictAllowlist`, and `DirectoryAdded`, v2.1.221 credential-file `mode:"mask"`, and v2.1.224 `archive`, `crossSessionInbound`, `dialogExpiry`, structured credential masking fields, `awsPairs`, and SigV4 policy enums. `workflowSizeGuideline` was already modeled from v2.1.202. 2.1.220 and 2.1.226 were bugfix-only; 2.1.225 had no static lint surface. Dropped owner-wildcard policy behavior, model-routing/runtime fixes, plugin activation, SendMessage, workspace-trust, UI, CLI, env-var, and other runtime-only bullets. Hook stdout contracts and command-body syntax remain excluded by the scope guardrail. Verified with `go build ./... && go test ./internal/cue/ ./internal/lint/` (commit 3f8c4b5).
- v2.1.227–v2.1.228 swept (2026-08-11). Installed binary is v2.1.228. Bugfix-only gap, NO lint surface. v2.1.228 (17 bullets) is entirely runtime/UI/runner bugfixes (interactive-session redraw, Windows `git`/Git Bash discovery, `/tui` model revert, cross-session inbox, Remote Control `/resume`, self-hosted-runner checkout/gap, memory-folder + plugin-cache cleanup, marketplace-entry merge SEMANTICS over the already-modeled `#MarketplaceSource`, deferred-tools reminder dedupe, claude.ai-synced-skill runtime hardening, cross-session message UI, Vertex AI credential handling, compaction progress UI, spinner glyphs, Write-tool read-tracking runtime behavior change, first-use notice text); v2.1.227 (5 bullets) is feature-flag subscription-tier eval, `claude-code-action` `allowed_non_write_users`, `/tui` rewind, slash-command menu styling, event-loop perf. The two schema-adjacent bullets are behavioral, not new surface: the Write-tool change is a runtime read-tracking rule (Write is already a KnownTool), and the marketplace change is merge semantics over the existing `#MarketplaceSource` schema. Classified against the full lintable surface: NO new settings.json keys, KnownTools, hook events/handler fields, model aliases, enum values, or plugin/agent/command/skill frontmatter fields. Docs-only sweep record; no code change.
|-----|-------|
| claude_code_last_updated | v2.1.228 (binary installed: v2.1.228) |
